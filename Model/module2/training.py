...
# Create an object similar to what parse_args() would return
opt = Namespace(**args_dict)

opt.dataroot = '/content/drive/MyDrive'
...
# Networks
netG_A2B = Generator_S2F.from_file('/content/drive/MyDrive/model_istda_lgsn_module1/netG_A2B.pth')  # shadow to shadow_free
netG_B2A = Generator_F2S.from_file('/content/drive/MyDrive/model_istda_lgsn_module1/netG_B2A.pth')  # shadow_free to shadow
...
for epoch in range(opt.epoch, opt.n_epochs):
    for i, batch in enumerate(dataloader):
        ...
        if (i+1) % opt.iter_loss == 0:
            ...
            avg_log = '[the last epoch], [loss_G %.5f], [D_A_losses %.5f], [D_B_losses %.5f],' \
                      % (G_losses[G_losses.__len__()-1], D_A_losses[D_A_losses.__len__()-1], \
            ...
            # img_name = 'out_image'
            # save_result = os.path.join(output_directory, f'{img_name}.jpg')
            # io.imsave(save_result, (outputimagerealsr * 255).astype(np.uint8))
